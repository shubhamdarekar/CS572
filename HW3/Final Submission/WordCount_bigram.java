      ...
      public void map(Object key, Text value, Context context) throws IOException, InterruptedException 
      {
         ...
         if(itr.hasMoreTokens()){
            ...
            while(itr.hasMoreTokens()){
               ...
               String combine = word.toString() + " " + word2.toString();

               if(Arrays.asList(bigrams).contains(combine)){

                  mapDetails.set(parts[0] + ":" + 1);

                  context.write(new Text(combine), mapDetails);
         ...
         //       {
         //           word2.set(itr.nextToken());
         //           String combine = word.toString().trim() + " " + word2.toString().trim();
         //           if(Arrays.asList(bigrams).contains(combine)){
         //               mapDetails.set(parts[0] + ":" + 1);
         //                  word.set(new Text(combine));
